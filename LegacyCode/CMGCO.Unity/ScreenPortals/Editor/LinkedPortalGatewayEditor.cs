...
namespace CMGCO.Unity.ScreenPortals
{
    ...
    public class LinkedPortalGatwayEditor : Editor
    {
        ...
        private ExitPortalResult exitPortalResult = new ExitPortalResult();

        private CustomGUIResult<AspectRatioIDs, Vector2> aspectRatioResult;
        private AnchoredWidthHeightResult screenSizeResult = new AnchoredWidthHeightResult(new Rect(), Anchors.NONE, false);
        ...
        private void getPropertiesFromSerializedObject()
        {
            ...
            GameObject exitPortal = (GameObject)serializedObject.FindProperty("exitPortal").objectReferenceValue;
            if (exitPortal)
            {
                // Get current result for exitPortalResult;
                LinkedPortalGateway exitPortalScript = this.myLinkedPortalGateway._exitPortal.GetComponent<LinkedPortalGateway>();
                this.exitPortalResult = new ExitPortalResult(exitPortal, exitPortalScript, this.exitPortalResult._hasChanged);
            }
            ...
            if (this.exitPortalResult.resultValue)
            {
                ...
                this.screenSizeResult = new AnchoredWidthHeightResult(serializedScreenSizeValue, Anchors.NONE, false);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!this.exitPortalResult._resultValue)
            {
        ...
        private void drawExitPortalInput()
        {
            ...
            ExitPortalResult newExitPortalResult = NewExitPortalGUI._instance.drawGUIControl(
                this.exitPortalResult,
                this.myLinkedPortalGateway.gameObject,
                "Exit Portal"
            );
            if (newExitPortalResult._hasChanged)
            {
                newExitPortalResult.ResetHasChanged();
                if (this.exitPortalResult._resultValue)
                {
                    this.setExitPortal(newExitPortalResult);
                }
                else
                {
                    this.clearExitPortal();
                }
                this.exitPortalResult = newExitPortalResult;
        ...
        private void clearExitPortal()
        {
            ...
            LinkedPortalGateway[] recordObject = this.exitPortalResult._resultScript ? new LinkedPortalGateway[] { this.myLinkedPortalGateway, this.exitPortalResult._resultScript } : new LinkedPortalGateway[] { this.myLinkedPortalGateway };
            Undo.RecordObjects(recordObject, undoName);
            this.myLinkedPortalGateway.clearExitPortal(true);
            Undo.CollapseUndoOperations(group);
        }

        private void setExitPortal(ExitPortalResult newExitPortalResult)
        {
            // This is confusing and hard to understand. 

            // Get local versions of all the exit portal scritps 
            LinkedPortalGateway currentExitPortalScript = this.exitPortalResult._resultScript;
            LinkedPortalGateway newExitPortalScript = newExitPortalResult._resultScript;
            LinkedPortalGateway newExitPortalLinkedScript = newExitPortalScript.exitPortalScript;
            bool isNewExitPortalLinked = newExitPortalLinkedScript != null;

            int group = Undo.GetCurrentGroup();
            string undoName = "Set Exit Portal";
            Undo.SetCurrentGroupName(undoName);
            LinkedPortalGateway[] recordObject;
            if (isNewExitPortalLinked)
            {
                recordObject = currentExitPortalScript ?
                    new LinkedPortalGateway[] { this.myLinkedPortalGateway, currentExitPortalScript, newExitPortalScript, newExitPortalLinkedScript }
                    : new LinkedPortalGateway[] { this.myLinkedPortalGateway, newExitPortalScript, newExitPortalLinkedScript };
            }
            else
            {
                recordObject = currentExitPortalScript ?
                   new LinkedPortalGateway[] { this.myLinkedPortalGateway, currentExitPortalScript, newExitPortalScript }
                   : new LinkedPortalGateway[] { this.myLinkedPortalGateway, newExitPortalScript };
            }

            Undo.RecordObjects(recordObject, undoName);
            this.myLinkedPortalGateway._exitPortal = newExitPortalResult._resultValue;

            if (!isNewExitPortalLinked || EditorUtility.DisplayDialog("Inherit Properties from?", "Which Portal would you like to inherit the linked properties from?", "This portal", "Exit portal"))
            {
